...
fn default_low_iv() -> f64 { 20.0 }
fn default_high_iv() -> f64 { 40.0 }

pub fn scan(store: &ReplayStore, request: &RegimeScanRequest) -> anyhow::Result<RegimeScanReport> {
    anyhow::ensure!(request.low_iv > 0.0 && request.high_iv > request.low_iv, "invalid IV regime thresholds");
    let report = run_backtest(store, &request.backtest)?;
    let mut buckets = Vec::new();

    for (label, predicate) in [
        ("low", Box::new(|t: &BacktestTrade| t.entry_atm_iv.is_some_and(|iv| iv < request.low_iv)) as Box<dyn Fn(&BacktestTrade)->bool>),
        ("mid", Box::new(|t: &BacktestTrade| t.entry_atm_iv.is_some_and(|iv| iv >= request.low_iv && iv < request.high_iv))),
        ("high", Box::new(|t: &BacktestTrade| t.entry_atm_iv.is_some_and(|iv| iv >= request.high_iv))),
    ] {
        buckets.push(bucket("atm_iv", label, report.trades.iter().filter(|t| predicate(t)).collect()));
    }

    for (label, predicate) in [
        ("negative", Box::new(|t: &BacktestTrade| t.entry_net_gex.is_some_and(|g| g < 0.0)) as Box<dyn Fn(&BacktestTrade)->bool>),
        ("positive", Box::new(|t: &BacktestTrade| t.entry_net_gex.is_some_and(|g| g >= 0.0))),
    ] {
        buckets.push(bucket("net_gex", label, report.trades.iter().filter(|t| predicate(t)).collect()));
    }
    ...
    for label in ["above", "below"] {
        buckets.push(bucket(
            "gamma_flip",
            label,
            report.trades.iter().filter(|t| t.spot_vs_gamma_flip.as_deref() == Some(label)).collect(),
        ));
    }
...
fn bucket(dimension: &str, name: &str, trades: Vec<&BacktestTrade>) -> RegimeBucket {
    ...
    RegimeBucket {
        ...
        bucket: name.into(),
        trades: count,
        win_rate: if count == 0 { 0.0 } else { wins as f64 / count as f64 },
        average_pnl: if count == 0 { 0.0 } else { total_pnl / count as f64 },
        total_pnl,
    }
}
